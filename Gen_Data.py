...
import csv
from Detectar_bordes import get_object_coordinates
...
def save_image():
    ...
    class_directory = os.path.join(output_dir, class_name)

    # Archivo CSV donde se guardan las coordenadas de los bordes detectados
    csv_file = open(os.path.join(output_dir, "bordes.csv"), mode='w', newline='')
    csv_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    csv_writer.writerow(['imagen', 'x', 'y', 'w', 'h'])


    frame, x_norm, y_norm, w_norm, h_norm = get_object_coordinates(frame)


    # Configura el directorio de la clase a guardar
    class_directory = os.path.join(output_dir, class_name)

    # Escribir la imagen con el rectángulo delimitador
    cv2.imwrite(os.path.join(class_directory, filename), frame)
    csv_writer.writerow([filename, x_norm, y_norm, w_norm, h_norm])
    csv_file.close()
